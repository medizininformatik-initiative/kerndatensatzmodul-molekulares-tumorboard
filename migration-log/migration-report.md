...
- **Not checked by this migration:** clinical correctness of any prose, the DNPM→MII StructureMaps' transformation correctness (the FML under `input/maps/` is carried unchanged and no longer compiled by the IG build — see QA-4), Unchanged from the source, out of scope here. (The Simplifier live-preview's equality with the git tree WAS subsequently checked on 2026-09-04 at the user's prompt: page tree and prose match exactly, including the source's typos; 0 preview-only pages — run.log `5.1c preview-crosscheck`. The preview nav also mirrors the source's own toc gaps, incl. a broken `Plodie-Observation.page.mdg` toc entry; the 10 toc-orphan pages were migrated regardless.)
