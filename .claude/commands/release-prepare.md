...
You are helping prepare a new release for the MII Modul Molekulares Tumorboard following the official MII Module Release Workflow.
...
Example: `/release-prepare 2026.0.0-rc.1`
...
1. **package.json**:
   ...
   - Update `description` with version highlights and link to release notes:
     ```
     "description": "Medizininformatik Initiative - Modul Molekulares Tumorboard. Version X.X.X: [key changes]. Release Notes: https://simplifier.net/guide/MII-IG-MTB-DE/MIIIGModulMolekularesTumorboard/ReleaseNotes"
     ```
2. **sushi-config.yaml**: Update the `version` parameter
3. **input/fsh/rulesets/version.fsh**: Update version in both RuleSets (if exists)
...
1. Find release notes: `implementation-guides/ImplementationGuide-2026.x-DE/MIIIGModulMolekularesTumorboard/ReleaseNotes.page.md`
...
- [ ] `input/fsh/rulesets/version.fsh` - both RuleSets (if exists)
