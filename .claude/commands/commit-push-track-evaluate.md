...
3. **Monitor CI validation**:
   ...
   RUN_ID=$(gh api repos/medizininformatik-initiative/kerndatensatzmodul-molekulares-tumorboard/actions/runs \
     --jq ".workflow_runs | map(select(.head_sha == \"$COMMIT_SHA\")) | .[0].id")

   # Poll until complete (use run_in_background for this)
   sleep 30 && gh api repos/medizininformatik-initiative/kerndatensatzmodul-molekulares-tumorboard/actions/runs/$RUN_ID \
...
5. **Run the analysis script** (if available):
   ```bash
