# Security and Privacy - MII IG Kerndatensatz-Modul Molekulares Tumorboard v2027.0.0-ballot.1
...
> **Written during migration - review before release.** The source guide ships no security-and-privacy section; the following module-specific aspects are PROPOSED from the module's data categories and require review by the module team (Gate B): this module carries **genomic data** (somatic and potentially germline variants from NGS panel/exome/genome sequencing) documented under the Modellvorhaben Genomsequenzierung (§ 64e SGB V). Genomic raw and variant data have a high intrinsic re-identification potential that profile-level pseudonymisation does not remove, and germline findings can carry information about biological relatives. Implementers SHOULD treat the variant-level resources (Einfache Variante, Copy Number Variant, DNA/RNA fusion, molecular biomarkers) as especially sensitive in DIMP configurations, and SHOULD take the DNPM/MV-GenomSeq governance rules into account when data leaves the site.
