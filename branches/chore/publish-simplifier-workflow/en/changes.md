# Changelog - MII IG Kerndatensatz-Modul Molekulares Tumorboard v2027.0.0-ballot.1
...
* Panel catalog: 11 `DeviceDefinition` templates for NGS panel products, a panel-capability CodeSystem/ValueSet pair, a gene-list extension and nine HGNC-ID-coded gene-list ValueSets, plus the **Panel-Katalog** page with a community feedback call.
* New pages: guidance for researchers (incl. OS/PFS/TTNT endpoint derivation), application scenarios (Kim Musterperson), DNPM transformation, security & privacy; complete English default language with German as full translation.
* `mii-vs-mtb-interpretation-high-low` (replacement for the ValueSet removed from Genomics Reporting STU3); NCIt topic **Tumor Board Review** (C93304) on the ImplementationGuide.

**Changed**

* Migrated onto the MII KDS module template (topic menu, generated artifact pages with the former Simplifier page texts as intros, template CI incl. SU-TermServ validation).
* All MII dependencies on their final 2027.0.0-ballot versions (molgen and onkologie on their `.1` fixes); `de.basisprofil.r4` 1.6.0; Genomics Reporting 3.0.0.
* `Procedure.basedOn` of both systemic-therapy profiles now constrains the oncology parent's inherited `tumorkonferenz` slice to `MII_PR_MTB_Therapieplan` (onkologie 2027 breaking change).
* IG resource id renamed to `mii-ig-mtb` (template pattern; packageId and canonical unchanged); three artifact ids aligned to their canonical tails; the logical model's canonical dropped the non-standard `/LogicalModel/` path segment.
* QA: 1,387 errors (pre-migration) → 55 on the MII terminology server, 0 broken links; the remaining classes are documented on the implementer-guidance page.

**Removed**

* `de.medizininformatikinitiative.kerndatensatz.consent` dependency (no artifact consumes it — the module's consent profile derives from plain `Observation`).
* Committed validation reports of the pre-migration setup (regenerated by CI at runtime).
