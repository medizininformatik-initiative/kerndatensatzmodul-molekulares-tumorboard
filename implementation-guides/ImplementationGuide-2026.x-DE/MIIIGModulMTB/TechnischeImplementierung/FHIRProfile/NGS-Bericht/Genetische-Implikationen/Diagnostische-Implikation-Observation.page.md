...
Das Profil beschreibt die Annotation der Therapiemöglichkeiten auf Basis molekularer Merkmale (z.B. genetische Varianten, immunhistochemische Nachweise ).
Hierbei geht es insbesondere um die vermuteten Pathogenitäten der vorliegenden Varianten / Genotypen. Eine Implikation kann sich aus einzelnen oder der Kombination von mehreren Varianten / Biomarkern ergeben. 
...
for
    differential.element
    where
        id.contains('DiagnostischeImplikation') and mapping.first().map.contains('MII_PR_MolGen_DiagnostischeImplikation')
    select 
        Datensatz: short,
...
<tabs>
    <tab title="Tabelle">{{table:Observation/MII-EXA-MTB-Diagnostische-Implikation-1}}</tab>
    <tab title="JSON">{{json:Observation/MII-EXA-MTB-Diagnostische-Implikation-1}}</tab>
</tabs>
