---
parent: 
topic: Einfache Variante
subject: https://www.medizininformatik-initiative.de/fhir/ext/modul-mtb/StructureDefinition/mii-pr-mtb-einfache-variante
---
...
for
    differential.element
    where
        id.contains('EinfacheVariante') and mapping.first().map.contains('MII_PR_MTB_Einfache_Variante')
    select 
        Datensatz: short,
...
<tabs>
    <tab title="Tabelle">{{table:Observation/MII-EXA-MTB-Einfache-Variante}}</tab>
    <tab title="JSON">{{json:Observation/MII-EXA-MTB-Einfache-Variante}}</tab>
</tabs>
