...
parent:
topic: BRCAness
subject: https://www.medizininformatik-initiative.de/fhir/ext/modul-mtb/StructureDefinition/mii-pr-mtb-brcaness
---
...
for
    differential.element
    where
        id.contains('BRCAness') and mapping.first().map.contains('MII_PR_MTB_BRCAness')
    select 
        Datensatz: short,
...
<tabs>
    <tab title="Tabelle">{{table:Observation/MII-EXA-MTB-BRCAness-1}}</tab>
    <tab title="JSON">{{json:Observation/MII-EXA-MTB-BRCAness-1}}</tab>
</tabs>
