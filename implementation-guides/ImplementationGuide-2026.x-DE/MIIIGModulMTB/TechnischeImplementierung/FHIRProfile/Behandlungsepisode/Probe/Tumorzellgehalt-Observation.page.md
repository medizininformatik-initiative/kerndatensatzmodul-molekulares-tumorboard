---
parent: 
topic: Tumorzellgehalt: Observation
subject: https://www.medizininformatik-initiative.de/fhir/ext/modul-mtb/StructureDefinition/mii-pr-mtb-tumorzellgehalt
---
...
from 
    StructureDefinition 
where 
    url.endsWith('mii-pr-mtb-tumorzellgehalt')
select 
    Name: name,
    Status: status,
    Version: version,
...
<tabs>
  <tab title="Darstellung">{{tree:https://www.medizininformatik-initiative.de/fhir/ext/modul-mtb/StructureDefinition/mii-pr-mtb-tumorzellgehalt, buttons}}</tab>
...
where url.endsWith('mii-lm-mtb')
    for
        differential.element
    where
        id.lower().contains('tumorzellgehalt') and 
...
<tabs>
    <tab title="Tabelle">{{table:mii-exa-mtb-kim-tumorzellgehalt-aszites}}</tab>
    <tab title="JSON">{{json:mii-exa-mtb-kim-tumorzellgehalt-aszites}}</tab>
</tabs>

---

{{render:mii-cs-mtb-bestimmungsmethode-tumorzellgehalt}}