...
where url = 'https://www.medizininformatik-initiative.de/fhir/ext/modul-mtb/StructureDefinition/LogicalModel/mii-lm-mtb'
    ...
    where
        id.contains('HumangenetischeBeratungAuftrag') and 
...
Beispiel: Auftrag zur Human-genetischen Beratung für Kim Musterperson vom 28.03.2023 (siehe {{pagelink:Szenarien}})

<tabs>
    <tab title="Tabelle">{{table:mii-exa-mtb-kim-humangenetische-beratung-aszites}}</tab>
    <tab title="JSON">{{json:mii-exa-mtb-kim-humangenetische-beratung-aszites}}</tab>
</tabs>
