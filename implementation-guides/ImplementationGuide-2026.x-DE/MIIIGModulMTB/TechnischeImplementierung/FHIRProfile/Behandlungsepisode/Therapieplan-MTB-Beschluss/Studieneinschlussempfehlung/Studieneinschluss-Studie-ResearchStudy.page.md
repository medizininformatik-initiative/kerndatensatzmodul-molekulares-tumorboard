---
parent: 
topic: Studieneinschluss (Studie)
subject: https://www.medizininformatik-initiative.de/fhir/ext/modul-mtb/StructureDefinition/mii-pr-mtb-studie
---
...
from 
    StructureDefinition 
where 
    url.endsWith('mii-pr-mtb-studie')
select 
    Name: name,
    Status: status,
    Version: version,
...
where url.endsWith('mii-lm-mtb')
    for
        differential.element
    where
        id.lower().contains('studieneinschlussempfehlung') and 
        mapping.first().map.contains('ResearchStudy')
    select 
        Datensatz: short,
...
Beispiel: Empfohlene Studie für Kim Musterperson vom 28.03.2023 (siehe {{pagelink:Szenarien}})

<tabs>
    <tab title="Tabelle">{{table:ResearchStudy/mii-exa-mtb-study-cldn6}}</tab>
    <tab title="JSON">{{json:ResearchStudy/mii-exa-mtb-study-cldn6}}</tab>
</tabs>
