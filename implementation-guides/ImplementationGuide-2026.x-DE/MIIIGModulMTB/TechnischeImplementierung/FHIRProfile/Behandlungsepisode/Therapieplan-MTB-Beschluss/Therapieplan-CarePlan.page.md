...
parent: 
topic: Therapieplan
subject: https://www.medizininformatik-initiative.de/fhir/ext/modul-mtb/StructureDefinition/mii-pr-mtb-therapieplan
---
...
from 
    StructureDefinition 
where 
    url.endsWith('mii-pr-mtb-therapieplan')
select 
    Name: name,
    Status: status,
    Version: version,
...
from
    StructureDefinition 
where
    url.endsWith('mii-lm-mtb')
for
    differential.element
    where
        id.lower().contains('beschlusstherapieplan') and mapping.first().map.contains('CarePlan')
    select 
        Datensatz: short,
