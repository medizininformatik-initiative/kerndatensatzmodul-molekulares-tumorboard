...
{{tree:https://dnpm-dip.net/fhir/StructureDefinition/dnpm-lm-patient-record, snapshot}}
...
for
    ...
    where mapping.where(identity = 'dnpm-to-mii-fhir').map.empty().not()
select
    'DNPM Element': path.replace('dnpm-lm-patient-record.', ''),
    'MII FHIR Ziel': mapping.where(identity = 'dnpm-to-mii-fhir').map.first()
</fql>
